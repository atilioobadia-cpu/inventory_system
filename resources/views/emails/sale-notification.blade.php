...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    <div style="padding: 20px; background: #f8fafc;">
        <h2 style="color: #10B981;">&#10003; New Sale Completed</h2>
        <p style="color: #334155; font-size: 14px; line-height: 1.6;">A new sale has been processed successfully. Below are the details:</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 16px 0; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
            <tr style="background: #1E293B;">
                <td colspan="2" style="padding: 10px 16px; color: white; font-weight: bold; font-size: 14px;">Sale Information</td>
            </tr>
            <tr>
            ...
            <tr>
                <td style="padding: 10px 16px; font-weight: bold; color: #475569; border-bottom: 1px solid #e2e8f0;">Payment Status</td>
                <td style="padding: 10px 16px; border-bottom: 1px solid #e2e8f0;">
                    <span style="display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; color: #ffffff; background: {{ $sale->payment_status === 'paid' ? '#10B981' : ($sale->payment_status === 'partial' ? '#F59E0B' : '#EF4444') }};">
                        {{ ucfirst($sale->payment_status) }}
                    </span>
                </td>
            </tr>
            <tr>
                <td style="padding: 10px 16px; font-weight: bold; color: #475569;">Cashier</td>
                <td style="padding: 10px 16px; color: #1E293B;">{{ $sale->user->name ?? 'N/A' }}</td>
            </tr>
        </table>

        <p style="color: #475569; font-size: 13px; font-weight: bold; margin: 16px 0 8px 0;">Items Sold:</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 16px 0; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
            ...
            <tr>
                ...
                <td style="padding: 10px 16px; color: #EF4444; text-align: right; border-bottom: 1px solid #e2e8f0;">- TZS {{ number_format($sale->discount, 2) }}</td>
            </tr>
            @endif
            @if(isset($sale->tax) && $sale->tax > 0)
            <tr>
                <td style="padding: 10px 16px; color: #475569; border-bottom: 1px solid #e2e8f0;">Tax</td>
                <td style="padding: 10px 16px; color: #1E293B; text-align: right; border-bottom: 1px solid #e2e8f0;">TZS {{ number_format($sale->tax, 2) }}</td>
            </tr>
            @endif
            <tr style="background: #10B981;">
                <td style="padding: 12px 16px; color: white; font-weight: bold; font-size: 16px;">Total Amount</td>
                <td style="padding: 12px 16px; color: white; font-weight: bold; font-size: 16px; text-align: right;">TZS {{ number_format($sale->total_amount, 2) }}</td>
            </tr>
        </table>

        <div style="text-align: center; margin: 24px 0;">
            <a href="{{ url('/sales/' . $sale->id) }}" style="display: inline-block; background: #10B981; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">View Sale Details</a>
        </div>
    </div>
