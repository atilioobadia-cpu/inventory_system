...
<span class="mx-2">/</span>
<span class="text-gray-900">{{ $supplier->name ?? 'Details' }}</span>
@endsection
        ...
        <div class="lg:col-span-1 space-y-4">
            ...
            <div class="card card-body">
                <div class="text-center mb-4">
                    ...
                    @else
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-white text-gray-700 mt-1">Inactive</span>
                    @endif
                </div>
                <div class="space-y-3 border-t border-border pt-4">
                    ...
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-gray-700">{{ $supplier->contact_person }}</span>
                    </div>
                    @endif
                    @if($supplier->phone)
                    <div class="flex items-center gap-2 text-sm">
                    ...
                    <div class="flex items-start gap-2 text-sm">
                        ...
                        <span class="text-gray-700">{{ $supplier->address }}{{ $supplier->city ? ', ' . $supplier->city : '' }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card card-body">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-900">TZS {{ number_format($supplier->credit_limit ?? 0) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Payment Terms</span>
                        <span class="font-medium text-gray-900">{{ ucfirst(str_replace('net', 'Net ', $supplier->payment_terms ?? 'cash')) }}</span>
                    </div>
                    @if($supplier->tin_number)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">TIN</span>
                        <span class="font-medium text-gray-900 font-mono">{{ $supplier->tin_number }}</span>
                    </div>
                    @endif
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            <tr class="hover:bg-white">
                                <td class="py-3 font-medium text-gray-900">{{ $purchase->invoice_number }}</td>
                                <td class="py-3 text-gray-700">{{ $purchase->created_at->format('d M Y') }}</td>
                                <td class="py-3 text-right text-gray-700">{{ $purchase->items_count ?? $purchase->items->count() }}</td>
                                <td class="py-3 text-right font-medium text-gray-900">TZS {{ number_format($purchase->total_amount) }}</td>
                                <td class="py-3 text-center">
                                    ...
                                    @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-white text-gray-700">{{ $purchase->status }}</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-gray-500">No purchase history</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
