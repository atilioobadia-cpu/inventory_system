...
<span class="mx-2">/</span>
<a href="{{ route('suppliers.show', $supplier) }}" class="hover:text-electric transition-colors">{{ $supplier->name }}</a>
<span class="mx-2">/</span>
...
<div class="max-w-3xl">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-3">
            <a href="{{ route('suppliers.show', $supplier) }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</a>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="address" rows="2"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric resize-none">{{ old('address', $supplier->address) }}</textarea>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric resize-none">{{ old('notes', $supplier->notes) }}</textarea>
                @error('notes')<p class="text-xs text-danger mt-1">{{ $message }}</p>@enderror
            </div>
        </div>

        <div class="flex justify-end gap-3 mt-6">
            <a href="{{ route('suppliers.show', $supplier) }}" class="px-6 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</a>
            <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-electric rounded-lg hover:bg-blue-600 transition-colors">Update Supplier</button>
        </div>
    </form>
</div>
@endsection