...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Role: {{ $role->name }}</h1>
            <p class="text-gray-500 mt-1">{{ $role->description ?? 'No description provided' }}</p>
        </div>
        <div class="flex items-center gap-3">
            @if(!$role->is_system)
                <a href="{{ route('roles.edit', $role) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-tz-green text-white rounded-lg hover:bg-tz-green-dark transition-colors text-sm font-medium">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <p class="font-medium text-gray-800 font-mono">{{ $role->slug }}</p>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Permissions ({{ $role->permissions->count() }})</h2>
        </div>
        <div class="p-6">
            @php
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Users with this Role ({{ $role->users->count() }})</h2>
        </div>
