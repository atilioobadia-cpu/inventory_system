    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($roles ?? [] as $role)
            <div class="bg-white rounded-lg border border-border p-5 transition-colors">
                ...
                <div class="flex items-center gap-2 pt-3 border-t border-border">
                    ...
                    @if(!$role->is_system)
                        <a href="{{ route('roles.edit', $role) }}" class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-sm text-muted hover:text-warning hover:bg-warning-light rounded-lg transition-colors" title="Edit">
                        ...
                        <form action="{{ route('roles.destroy', $role) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this role?')" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-sm text-muted hover:text-danger hover:bg-danger-light rounded-lg transition-colors" title="Delete">
        ...
        @empty
            <div class="col-span-full py-16 text-center">
                <svg class="w-16 h-16 text-muted/50 mx-auto mb-4" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/></svg>
                <p class="text-muted font-medium">No roles found</p>
                <p class="text-sm text-muted mt-1">Create your first role to get started.</p>
            </div>
        @endforelse
    </div>
</div>
