...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('roles.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="text-left px-6 py-3 font-medium text-slate-500">Name</th>
                        <th class="text-left px-6 py-3 font-medium text-slate-500">Slug</th>
                        <th class="text-center px-6 py-3 font-medium text-slate-500">Permissions</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($roles ?? [] as $role)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-slate-500 font-mono text-xs">{{ $role->slug }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">{{ $role->permissions_count ?? $role->permissions->count() }} permissions</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded-full text-xs font-medium">{{ $role->users_count ?? $role->users->count() }} users</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($role->is_system)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('roles.show', $role) }}" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    </a>
                                    @if(!$role->is_system)
                                        <a href="{{ route('roles.edit', $role) }}" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
