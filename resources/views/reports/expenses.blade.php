...
<div class="space-y-6" x-data="expenseReport()">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-heading">Expense Report</h1>
            <p class="text-muted mt-1">Track and analyze business expenses</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.expenses') }}?{{ http_build_query(array_merge(request()->query(), ['export' => 'csv'])) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors text-sm font-medium">
