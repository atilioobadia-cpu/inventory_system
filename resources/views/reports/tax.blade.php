...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.tax') }}?{{ http_build_query(array_merge(request()->query(), ['export' => 'csv'])) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors text-sm font-medium">
                    ...
                    @forelse($monthlyVat ?? [] as $month)
                        <tr class="hover:bg-card-bg">
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($month['net'] > 0)
                                    <span class="px-2 py-1 bg-accent-light text-blue-700 rounded-full text-xs font-medium">Payable</span>
                                @elseif($month['net'] < 0)
