...
<div class="space-y-6" x-data="inventoryReport()">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">Stock levels, valuations and category breakdown</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.inventory') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-tz-green text-white rounded-lg hover:bg-tz-green-dark transition-colors text-sm font-medium">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('reports.inventory') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div>
                ...
                <select name="status" class="rounded-lg border-gray-300 focus:border-tz-green focus:ring-tz-green/20 text-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="text-center">
                ...
                <p class="text-xl font-bold text-gray-800">{{ number_format($totalItems ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-500">Stock Value (Cost)</p>
                <p class="text-xl font-bold text-gray-800">TZS {{ number_format($totalStockValue ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-500">Retail Value</p>
                <p class="text-xl font-bold text-gray-800">TZS {{ number_format($totalRetailValue ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="text-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-800">Stock Details</h2>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 sticky top-0">
                        <tr>
                            ...
                            <th class="text-right px-4 py-3 font-medium text-gray-500">Cost</th>
                            <th class="text-right px-4 py-3 font-medium text-gray-500">Selling</th>
                            <th class="text-right px-4 py-3 font-medium text-gray-500">Value</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($items ?? [] as $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center font-medium {{ $item->quantity <= $item->low_stock_threshold ? ($item->quantity == 0 ? 'text-red-600' : 'text-amber-600') : 'text-gray-800' }}">{{ $item->quantity }}</td>
                                <td class="px-4 py-3 text-right text-gray-600">TZS {{ number_format($item->cost_price) }}</td>
                                <td class="px-4 py-3 text-right text-gray-600">TZS {{ number_format($item->selling_price) }}</td>
                                <td class="px-4 py-3 text-right font-medium text-gray-800">TZS {{ number_format($item->quantity * $item->cost_price) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-500">No inventory items found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-4 space-y-2">
                @foreach($categoryBreakdown ?? [] as $index => $cat)
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-800">{{ $cat['count'] }} items</span>
                    </div>
                @endforeach
            </div>
        </div>
