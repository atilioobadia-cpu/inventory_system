...
@section('content')
<div class="space-y-6" x-data="inventoryReport()">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Inventory Report</h1>
            <p class="text-slate-500 mt-1">Stock levels, valuations and category breakdown</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.inventory') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="text-center">
                ...
                <p class="text-xl font-bold text-slate-800">{{ number_format($totalItems ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="text-center">
                ...
                <p class="text-xl font-bold text-slate-800">TZS {{ number_format($totalStockValue ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="text-center">
                ...
                <p class="text-xl font-bold text-slate-800">TZS {{ number_format($totalRetailValue ?? 0) }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="text-center">
                <div class="p-2 bg-amber-100 rounded-lg inline-flex mb-2">
                    <svg class="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
                </div>
                <p class="text-sm text-slate-500">Low Stock</p>
                <p class="text-xl font-bold text-amber-600">{{ $lowStockItems ?? 0 }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="text-center">
                <div class="p-2 bg-red-100 rounded-lg inline-flex mb-2">
                ...
                <p class="text-xl font-bold text-red-600">{{ $outOfStockItems ?? 0 }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-200">
                <h2 class="text-lg font-semibold text-slate-800">Stock Details</h2>
            </div>
            <div class="overflow-x-auto max-h-[600px] overflow-y-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 sticky top-0">
                        <tr>
                            <th class="text-left px-4 py-3 font-medium text-slate-500">Item</th>
                            <th class="text-left px-4 py-3 font-medium text-slate-500">SKU</th>
                            <th class="text-left px-4 py-3 font-medium text-slate-500">Category</th>
                            <th class="text-center px-4 py-3 font-medium text-slate-500">Qty</th>
                            <th class="text-right px-4 py-3 font-medium text-slate-500">Cost</th>
                            <th class="text-right px-4 py-3 font-medium text-slate-500">Selling</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($items ?? [] as $item)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-slate-500 font-mono text-xs">{{ $item->sku }}</td>
                                <td class="px-4 py-3 text-slate-600">{{ $item->category->name ?? '-' }}</td>
                                <td class="px-4 py-3 text-center font-medium {{ $item->quantity <= $item->low_stock_threshold ? ($item->quantity == 0 ? 'text-red-600' : 'text-amber-600') : 'text-slate-800' }}">{{ $item->quantity }}</td>
                                <td class="px-4 py-3 text-right text-slate-600">TZS {{ number_format($item->cost_price) }}</td>
                                <td class="px-4 py-3 text-right text-slate-600">TZS {{ number_format($item->selling_price) }}</td>
                                <td class="px-4 py-3 text-right font-medium text-slate-800">TZS {{ number_format($item->quantity * $item->cost_price) }}</td>
                                <td class="px-4 py-3 text-center">
                                    ...
                                    @elseif($item->quantity <= $item->low_stock_threshold)
                                        <span class="px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-medium">Low Stock</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-slate-500">No inventory items found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <!-- Category Breakdown -->
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 class="text-lg font-semibold text-slate-800 mb-4">Category Breakdown</h2>
            <div class="h-72">
            ...
            <div class="mt-4 space-y-2">
                @foreach($categoryBreakdown ?? [] as $index => $cat)
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-medium text-slate-800">{{ $cat['count'] }} items</span>
                    </div>
                @endforeach
            </div>
        </div>
