...
<div class="space-y-6" x-data="salesReport()">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-heading">Sales Report</h1>
        ...
        <div class="flex items-center gap-3">
            <form action="{{ route('reports.export') }}" method="POST" class="inline-flex">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors text-sm font-medium">
    ...
    <div class="bg-white rounded-xl border border-border p-6">
        <form action="{{ route('reports.sales') }}" method="GET" class="flex flex-wrap items-end gap-4">
            <div>
                ...
                <input type="date" name="from" value="{{ request('from', now()->startOfMonth()->format('Y-m-d')) }}" class="rounded-lg border-border focus:border-accent focus:ring-accent/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-body mb-1">To Date</label>
                <input type="date" name="to" value="{{ request('to', now()->format('Y-m-d')) }}" class="rounded-lg border-border focus:border-accent focus:ring-accent/20 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-body mb-1">Customer</label>
                <select name="customer_id" class="rounded-lg border-border focus:border-accent focus:ring-accent/20 text-sm min-w-[200px]">
            ...
            <a href="{{ route('reports.sales') }}" class="px-4 py-2 bg-control-bg text-gray-600 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">Reset</a>
        </form>
    </div>
