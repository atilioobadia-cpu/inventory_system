...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-heading">Customer Report</h1>
            <p class="text-muted mt-1">Customer sales and outstanding balances</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.customers') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors text-sm font-medium">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($customers ?? [] as $customer)
                        <tr class="hover:bg-card-bg">
                            ...
                            <td class="px-6 py-4 text-right {{ ($customer->balance ?? 0) > 0 ? 'text-danger font-semibold' : 'text-gray-600' }}">TZS {{ number_format($customer->balance ?? 0) }}</td>
