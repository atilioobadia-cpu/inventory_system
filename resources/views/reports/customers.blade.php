...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">Customer sales and outstanding balances</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.customers') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-tz-green text-white rounded-lg hover:bg-tz-green-dark transition-colors text-sm font-medium">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ number_format($totalCustomers ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="text-right px-6 py-3 font-medium text-gray-500">Total Sales</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($customers ?? [] as $customer)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-right font-medium text-gray-800">TZS {{ number_format($customer->total_sales ?? 0) }}</td>
                            <td class="px-6 py-4 text-right text-green-600">TZS {{ number_format($customer->total_paid ?? 0) }}</td>
                            <td class="px-6 py-4 text-right {{ ($customer->balance ?? 0) > 0 ? 'text-red-600 font-semibold' : 'text-gray-600' }}">TZS {{ number_format($customer->balance ?? 0) }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $customer->last_purchase?->format('d M Y') ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
