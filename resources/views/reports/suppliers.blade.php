...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Supplier Report</h1>
            <p class="text-slate-500 mt-1">Supplier purchases and outstanding balances</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.suppliers') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-slate-800">{{ number_format($totalSuppliers ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-200">
            <h2 class="text-lg font-semibold text-slate-800">Supplier Details</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="text-right px-6 py-3 font-medium text-slate-500">Total Purchases</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($suppliers ?? [] as $supplier)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-right font-medium text-slate-800">TZS {{ number_format($supplier->total_purchases ?? 0) }}</td>
                            <td class="px-6 py-4 text-right text-green-600">TZS {{ number_format($supplier->total_paid ?? 0) }}</td>
                            <td class="px-6 py-4 text-right {{ ($supplier->balance ?? 0) > 0 ? 'text-red-600 font-semibold' : 'text-slate-600' }}">TZS {{ number_format($supplier->balance ?? 0) }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $supplier->last_purchase?->format('d M Y') ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
