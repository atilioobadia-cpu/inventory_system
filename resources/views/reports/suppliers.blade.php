...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-heading">Supplier Report</h1>
            <p class="text-muted mt-1">Supplier purchases and outstanding balances</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.suppliers') }}?export=csv" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($suppliers ?? [] as $supplier)
                        <tr class="hover:bg-white">
                            ...
                            <td class="px-6 py-4 text-body">{{ $supplier->phone ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-right {{ ($supplier->balance ?? 0) > 0 ? 'text-danger font-semibold' : 'text-body' }}">TZS {{ number_format($supplier->balance ?? 0) }}</td>
                            <td class="px-6 py-4 text-body">{{ $supplier->last_purchase?->format('d M Y') ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
