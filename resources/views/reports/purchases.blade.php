...
<div class="space-y-6" x-data="purchaseReport()">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-500 mt-1">Track purchases and supplier spending</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.purchases') }}?{{ http_build_query(array_merge(request()->query(), ['export' => 'csv'])) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-slate-800">TZS {{ number_format($totalPurchases ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-500">Items Received</p>
                    <p class="text-xl font-bold text-slate-800">{{ number_format($itemsReceived ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-500">Average Purchase</p>
                    <p class="text-xl font-bold text-slate-800">TZS {{ number_format($averagePurchase ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-500">VAT Paid</p>
                    <p class="text-xl font-bold text-slate-800">TZS {{ number_format($vatPaid ?? 0) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-200">
            <h2 class="text-lg font-semibold text-slate-800">Purchase Details</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="text-left px-6 py-3 font-medium text-slate-500">PO Number</th>
                        <th class="text-left px-6 py-3 font-medium text-slate-500">Supplier</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($purchases ?? [] as $purchase)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 text-slate-600">{{ $purchase->date->format('d M Y') }}</td>
                            <td class="px-6 py-4 font-medium text-blue-600">{{ $purchase->po_number }}</td>
                            <td class="px-6 py-4 text-slate-800">{{ $purchase->supplier->name ?? '-' }}</td>
                            <td class="px-6 py-4 text-center text-slate-600">{{ $purchase->items_count }}</td>
                            <td class="px-6 py-4 text-right text-slate-600">TZS {{ number_format($purchase->subtotal) }}</td>
                            <td class="px-6 py-4 text-right text-slate-600">TZS {{ number_format($purchase->vat_amount) }}</td>
                            <td class="px-6 py-4 text-right font-semibold text-slate-800">TZS {{ number_format($purchase->total) }}</td>
