...
<div class="space-y-6" x-data="purchaseReport()">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">Track purchases and supplier spending</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('reports.purchases') }}?{{ http_build_query(array_merge(request()->query(), ['export' => 'csv'])) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-tz-green text-white rounded-lg hover:bg-tz-green-dark transition-colors text-sm font-medium">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('reports.purchases') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div>
                ...
                <select name="supplier_id" class="rounded-lg border-gray-300 focus:border-tz-green focus:ring-tz-green/20 text-sm min-w-[200px]">
            ...
            <a href="{{ route('reports.purchases') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">Reset</a>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-gray-500">Items Received</p>
                    <p class="text-xl font-bold text-gray-800">{{ number_format($itemsReceived ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-gray-500">Average Purchase</p>
                    <p class="text-xl font-bold text-gray-800">TZS {{ number_format($averagePurchase ?? 0) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-gray-500">VAT Paid</p>
                    <p class="text-xl font-bold text-gray-800">TZS {{ number_format($vatPaid ?? 0) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-800">Purchase Trend</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Purchase Details</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="text-left px-6 py-3 font-medium text-gray-500">PO Number</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($purchases ?? [] as $purchase)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 font-medium text-blue-600">{{ $purchase->po_number }}</td>
                            <td class="px-6 py-4 text-gray-800">{{ $purchase->supplier->name ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-right font-semibold text-gray-800">TZS {{ number_format($purchase->total) }}</td>
                            <td class="px-6 py-4 text-center">
                                @if($purchase->status === 'received')
                                    <span class="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">Received</span>
