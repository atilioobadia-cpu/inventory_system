...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
    <div class="flex items-center gap-4">
        <a href="{{ route('reconciliations.index') }}" class="p-2 rounded-lg text-muted hover:text-body hover:bg-white transition-colors">
        ...
        <div>
            <div class="flex items-center gap-3">
                <h2 class="text-xl font-bold text-heading">Reconciliation</h2>
                ...
                @else
                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-white text-body">Pending</span>
                @endif
            </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-lg border border-border p-5">
        ...
        <p class="text-xl font-bold text-heading">TZS {{ number_format($rec->expected_cash ?? 0, 2) }}</p>
    </div>
    <div class="bg-white rounded-lg border border-border p-5">
        <div class="flex items-center gap-3 mb-3">
        ...
        <p class="text-xl font-bold text-heading">TZS {{ number_format($rec->actual_cash ?? 0, 2) }}</p>
    </div>
    <div class="rounded-lg border p-6 {{ $diff != 0 ? 'bg-danger-light border-danger' : 'bg-success-light border-success' }}">
        <div class="flex items-center gap-3 mb-3">
        ...
        <p class="text-xl font-bold {{ $diff != 0 ? 'text-danger' : 'text-success' }}">{{ $diff >= 0 ? '+' : '' }}TZS {{ number_format($diff, 2) }}</p>
    </div>
</div>

@if($diff != 0)
<div class="bg-danger-light border border-danger rounded-lg p-4 mb-6 flex items-center gap-3">
...
<div class="bg-white rounded-lg border border-border mb-6">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-border">
                ...
                <tr class="hover:bg-white">
                    ...
                    <td class="px-6 py-3">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-white text-heading">{{ ucfirst($tx->type ?? 'other') }}</span>
                    </td>
