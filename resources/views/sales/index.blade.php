                        ...
                        <tr class="{{ $sale->is_voided ? 'bg-gray-50 opacity-60' : 'hover:bg-gray-50' }} transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1" x-data="{ open: false }">
                                    <button @click="open = !open" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-16 text-center">
                                ...
                                <p class="text-sm text-gray-400 mt-1">Sales will appear here after creating transactions via POS.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div @click.away="showVoidModal = false"
            ...
            <div class="p-6">
                ...
                <form :action="'{{ url('/sales') }}/' + voidSaleId + '/void'" method="POST" x-ref="voidForm">
                    @csrf
                    <div class="mb-4">
                        <label for="void_reason" class="block text-sm font-medium text-gray-700 mb-1">Reason for voiding <span class="text-red-500">*</span></label>
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 rounded-b-xl">
                <button @click="showVoidModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    Cancel
                </button>
                <button @click="$refs.voidForm.submit()" :disabled="!voidReason.trim()" x-bind:class="voidReason.trim() ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'"
