...
<div x-data="{
    ...
    <div class="card">
        <form method="GET" action="{{ route('sales.index') }}">
            <div class="p-4 border-b border-gray-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div class="lg:col-span-2">
                    <div class="relative">
                        <svg class="w-4 h-4 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"/>
                        </svg>
                        <input type="text" name="search" x-model="search" placeholder="Search by invoice, customer..." class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                </div>
                <div>
                    <input type="date" name="date_from" x-model="dateFrom" placeholder="From" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
                <div>
                    <input type="date" name="date_to" x-model="dateTo" placeholder="To" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
                <div>
                    <select name="customer_id" x-model="customer" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
            ...
            <div class="p-4 border-b border-gray-200 flex flex-wrap items-center gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <select name="status" x-model="status" class="px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($sales ?? [] as $sale)
                        <tr class="{{ $sale->is_voided ? 'bg-gray-50 opacity-60' : 'hover:bg-gray-50' }} transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500 text-center">{{ $sale->items_count ?? $sale->items->count() }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700 text-right">TZS {{ number_format($sale->subtotal, 2) }}</td>
                            <td class="px-4 py-3 text-sm text-gray-500 text-right">TZS {{ number_format($sale->tax_amount, 2) }}</td>
                            <td class="px-4 py-3 text-sm font-semibold text-gray-900 text-right">TZS {{ number_format($sale->total_amount, 2) }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1" x-data="{ open: false }">
                                    <button @click="open = !open" class="p-1.5 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-cloak
                                         ...
                                         class="absolute right-0 mt-2 w-48 bg-white rounded-lg border border-gray-200 py-1 z-20">
                                        <a href="{{ route('sales.show', $sale) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                        ...
                                        <button @click="$dispatch('view-receipt', { id: {{ $sale->id }} }); open = false"
                                                class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left">
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-16 text-center">
                                ...
                                <p class="text-sm text-gray-500 mt-1">Sales will appear here after creating transactions via POS.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div @click.away="showVoidModal = false"
            ...
            <div class="p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">Void Sale</h3>
                ...
                <form :action="'{{ url('/sales') }}/' + voidSaleId + '/void'" method="POST" x-ref="voidForm">
                    @csrf
                    <div class="mb-4">
                        <label for="void_reason" class="form-label">Reason for voiding <span class="text-red-600">*</span></label>
                        <textarea name="void_reason" x-model="voidReason" rows="3" required placeholder="Enter the reason for voiding this sale..."
                                  class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500"></textarea>
                    </div>
                </form>
            </div>
            <div class="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 rounded-b-lg">
                <button @click="showVoidModal = false" class="btn btn-secondary">
                    Cancel
                </button>
                <button @click="$refs.voidForm.submit()" :disabled="!voidReason.trim()" x-bind:class="voidReason.trim() ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-100 text-gray-400 cursor-not-allowed'"
