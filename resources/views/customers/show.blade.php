...
<span class="mx-2">/</span>
<span class="text-gray-900">{{ $customer->name ?? 'Details' }}</span>
@endsection
            ...
            <div class="card card-body">
                <div class="text-center mb-4">
                    ...
                    <div class="flex items-center justify-center gap-2 mt-1">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            ...
                            @elseif(($customer->customer_type ?? 'individual') === 'business') bg-gray-100 text-primary
                            @else bg-white text-gray-700 @endif">
                ...
                <div class="space-y-3 border-t border-border pt-4">
                    ...
                    <div class="flex items-start gap-2 text-sm">
                        <svg class="w-4 h-4 text-gray-500 mt-0.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/>
                        </svg>
                        <span class="text-gray-700">{{ $customer->address }}{{ $customer->city ? ', ' . $customer->city : '' }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-gray-700 font-mono text-xs">TIN: {{ $customer->tin_number }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card card-body">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium {{ ($customer->current_balance ?? 0) > 0 ? 'text-danger' : 'text-primary' }}">TZS {{ number_format($customer->current_balance ?? 0) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Credit Limit</span>
                        <span class="font-medium text-gray-900">TZS {{ number_format($customer->credit_limit ?? 0) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Total Purchases</span>
                        <span class="font-medium text-gray-900">TZS {{ number_format($totalPurchases ?? 0) }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            <tr class="hover:bg-white">
                                <td class="py-3 font-medium text-gray-900">{{ $sale->invoice_number }}</td>
                                <td class="py-3 text-gray-700">{{ $sale->created_at->format('d M Y') }}</td>
                                <td class="py-3 text-right text-gray-700">{{ $sale->items_count ?? $sale->items->count() }}</td>
                                <td class="py-3 text-right font-medium text-gray-900">TZS {{ number_format($sale->total_amount) }}</td>
                                <td class="py-3 text-right text-gray-700">TZS {{ number_format($sale->paid_amount ?? 0) }}</td>
                                <td class="py-3 text-center">
                                    ...
                                    @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-white text-gray-700">{{ $sale->payment_status }}</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-gray-500">No sales history</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
