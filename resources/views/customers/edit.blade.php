...
<span class="mx-2">/</span>
<a href="{{ route('customers.show', $customer) }}" class="hover:text-electric transition-colors">{{ $customer->name }}</a>
<span class="mx-2">/</span>
...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('customers.update', $customer) }}">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Customer Name <span class="text-danger">*</span></label>
            ...
            <div>
                ...
                <textarea name="address" rows="2"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric resize-none">{{ old('address', $customer->address) }}</textarea>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="type" value="individual" {{ old('type', $customer->type ?? 'individual') == 'individual' ? 'checked' : '' }}
                               class="w-4 h-4 text-electric border-gray-300 focus:ring-electric">
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric resize-none">{{ old('notes', $customer->notes) }}</textarea>
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-electric rounded-lg hover:bg-blue-600 transition-colors">Update Customer</button>
        </div>
    </form>
</div>
@endsection