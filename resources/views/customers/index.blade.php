...
<span class="mx-2">/</span>
<span class="text-gray-900">All Customers</span>
@endsection
...
<div x-data="{ deleteModal: false, deleteUrl: '', deleteName: '' }">
    <div class="page-header mb-6">
        <div class="page-title">
            ...
            <p class="page-subtitle">{{ $customers->total() ?? 0 }} customers</p>
        </div>
    ...
    <div class="card card-body mb-6">
        <form method="GET" action="{{ route('customers.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <select name="customer_type" class="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-gray-700">
            ...
            <table class="data-table">
                <thead class="bg-white border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-center px-4 py-3 font-medium text-gray-500">Type</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <a href="{{ route('customers.show', $customer) }}" class="font-medium text-gray-900 hover:text-primary">{{ $customer->name }}</a>
                        </td>
                        <td class="px-4 py-3 text-gray-700">{{ $customer->phone ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $customer->email ?? '-' }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="badge
                                {{ $customer->customer_type === 'wholesale' ? 'badge-yellow' : ($customer->customer_type === 'business' ? 'badge-gray' : 'badge-gray') }}">
                                {{ ucfirst($customer->customer_type ?? 'individual') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-right font-medium {{ ($customer->current_balance ?? 0) > 0 ? 'text-red-600' : 'text-gray-900' }}">
                            TZS {{ number_format($customer->current_balance ?? 0) }}
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('customers.show', $customer) }}" class="p-1.5 text-gray-500 hover:text-primary rounded-lg hover:bg-gray-100 transition-colors" title="View">
                                ...
                                @can('edit_customers')
                                <a href="{{ route('customers.edit', $customer) }}" class="p-1.5 text-gray-500 hover:text-primary rounded-lg hover:bg-gray-100 transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    <div x-show="deleteModal" x-cloak
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-lg max-w-md w-full p-5">
                ...
                <p class="text-sm text-gray-700 mb-6">Are you sure you want to delete <span class="font-semibold" x-text="deleteName"></span>? This will affect sales records.</p>
                <div class="flex justify-end gap-3">
                    <button @click="deleteModal = false" class="btn btn-secondary">Cancel</button>
