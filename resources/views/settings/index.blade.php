...
<div class="max-w-4xl mx-auto space-y-6" x-data="{ activeTab: '{{ request('tab', 'business') }}' }">
    ...
    <div class="border-b border-border">
        <nav class="flex gap-1 -mb-px">
            <button @click="activeTab = 'business'" :class="activeTab === 'business' ? 'border-blue-500 text-accent' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
            ...
            <button @click="activeTab = 'receipt'" :class="activeTab === 'receipt' ? 'border-blue-500 text-accent' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                    Receipt
                </span>
            </button>
            <button @click="activeTab = 'system'" :class="activeTab === 'system' ? 'border-blue-500 text-accent' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
            ...
            <button @click="activeTab = 'email'" :class="activeTab === 'email' ? 'border-blue-500 text-accent' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
    ...
    <div x-show="activeTab === 'business'" x-transition>
        <form action="{{ route('settings.update') }}" method="POST" class="bg-white rounded-xl border border-border p-6 space-y-6">
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors text-sm font-medium">Save Changes</button>
            </div>
        </form>
    </div>
