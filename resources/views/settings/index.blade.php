...
<div class="max-w-4xl mx-auto space-y-4" x-data="{ activeTab: '{{ request('tab', 'business') }}' }">
    ...
    <div class="border-b border-border">
        <nav class="flex gap-1 -mb-px">
            <button @click="activeTab = 'business'" :class="activeTab === 'business' ? 'border-primary text-primary' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
            ...
            <button @click="activeTab = 'receipt'" :class="activeTab === 'receipt' ? 'border-primary text-primary' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                    Receipt
                </span>
            </button>
            <button @click="activeTab = 'system'" :class="activeTab === 'system' ? 'border-primary text-primary' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
            ...
            <button @click="activeTab = 'email'" :class="activeTab === 'email' ? 'border-primary text-primary' : 'border-transparent text-muted hover:text-body'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
    ...
    <div x-show="activeTab === 'business'" x-transition>
        <form action="{{ route('settings.update') }}" method="POST" class="bg-white rounded-lg border border-border p-5 space-y-4">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div x-data="{ showLogo: {{ ($settings['receipt_show_logo'] ?? true) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-white rounded-lg">
                        ...
                        <button type="button" @click="showLogo = !showLogo" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="showLogo ? 'bg-gray-1000' : 'bg-white'">
            ...
            <div class="space-y-4">
                ...
                <div x-data="{ emailNotif: {{ ($settings['enable_email_notifications'] ?? true) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-white rounded-lg">
                        ...
                        <button type="button" @click="emailNotif = !emailNotif" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="emailNotif ? 'bg-gray-1000' : 'bg-white'">
                ...
                <div x-data="{ smsNotif: {{ ($settings['enable_sms_notifications'] ?? false) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-white rounded-lg">
                        ...
                        <button type="button" @click="smsNotif = !smsNotif" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="smsNotif ? 'bg-gray-1000' : 'bg-white'">
