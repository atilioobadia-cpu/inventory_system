...
<div class="max-w-4xl mx-auto space-y-4" x-data="{ activeTab: '{{ request('tab', 'business') }}' }">
    ...
    <div class="border-b border-gray-200">
        <nav class="flex gap-1 -mb-px">
            <button @click="activeTab = 'business'" :class="activeTab === 'business' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
            ...
            <button @click="activeTab = 'receipt'" :class="activeTab === 'receipt' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                    Receipt
                </span>
            </button>
            <button @click="activeTab = 'system'" :class="activeTab === 'system' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
            ...
            <button @click="activeTab = 'email'" :class="activeTab === 'email' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'" class="px-4 py-3 text-sm font-medium border-b-2 transition-colors">
                <span class="flex items-center gap-2">
            ...
            <div class="space-y-4">
                ...
                <div x-data="{ showLogo: {{ ($settings['receipt_show_logo'] ?? true) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div>
                            <p class="font-medium text-gray-900">Show Logo on Receipt</p>
                            <p class="text-sm text-gray-500">Display company logo at the top of receipts</p>
                        </div>
                        <button type="button" @click="showLogo = !showLogo" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="showLogo ? 'bg-primary' : 'bg-gray-300'">
        ...
        <form action="{{ route('settings.update') }}" method="POST" class="form-card">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="form-error">Items at or below this quantity will be flagged as low stock</p>
                </div>
                <div x-data="{ emailNotif: {{ ($settings['enable_email_notifications'] ?? true) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        ...
                        <button type="button" @click="emailNotif = !emailNotif" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="emailNotif ? 'bg-primary' : 'bg-gray-300'">
                ...
                <div x-data="{ smsNotif: {{ ($settings['enable_sms_notifications'] ?? false) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        ...
                        <button type="button" @click="smsNotif = !smsNotif" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="smsNotif ? 'bg-primary' : 'bg-gray-300'">
    ...
    <div x-show="activeTab === 'email'" x-transition>
        <form action="{{ route('settings.update') }}" method="POST" class="form-card">
            ...
            <h2 class="text-lg font-semibold text-gray-900">Email Notification Recipients</h2>
