...
<div class="space-y-8" x-data="{ importModal: false, importType: '', importing: false, importResult: null }">
    <div>
        ...
        <p class="text-slate-500 mt-1">Bulk import data from CSV files or export your data</p>
    </div>

    <!-- Import Section -->
    <div>
        <h2 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
            Import Data
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center gap-3 mb-3">
                    <div class="p-2 bg-blue-100 rounded-lg">
                        <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9"/></svg>
                    </div>
                    <div>
                        <h3 class="font-semibold text-slate-800">Items</h3>
                        <p class="text-xs text-slate-500">Products and parts</p>
                    </div>
                </div>
                <button @click="importType = 'items'; importModal = true; importResult = null" class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">Import</button>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Product categories</p>
                    </div>
                </div>
                <button @click="importType = 'categories'; importModal = true; importResult = null" class="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">Import</button>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Customer records</p>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-slate-800">Suppliers</h3>
                        <p class="text-xs text-slate-500">Supplier records</p>
                    </div>
                </div>
    ...
    <div>
        <h2 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
            Export Data
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            @php
            ...
            @foreach($exports as $export)
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-shadow">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <h3 class="font-semibold text-slate-800">{{ $export['name'] }}</h3>
                    </div>
                    <a href="{{ route($export['route'], ['type' => $export['type']]) }}" class="block w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium text-center">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative bg-white rounded-xl shadow-xl max-w-lg w-full p-6" @click.stop>
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-semibold text-slate-800 capitalize">Import <span x-text="importType"></span></h3>
                    <button @click="importModal = false" class="p-1 text-slate-400 hover:text-slate-600 rounded-lg">
                ...
                <!-- Error Results -->
                <div x-show="importResult && !importResult.success" class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                    <div class="flex items-center gap-2">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 bg-slate-50 rounded-lg">
                        <p class="text-sm text-slate-600 mb-2">1. Download the template CSV file:</p>
                        <span class="inline-flex items-center gap-2 text-sm text-slate-400 font-medium">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
                    ...
                    <form :action="'{{ url('/import-export/import') }}/' + importType" method="POST" enctype="multipart/form-data" @submit.prevent="submitImport($el)">
                        ...
                        <p class="text-sm text-slate-600 mb-2">2. Select your completed CSV or Excel file:</p>
                        <input type="file" name="file" accept=".csv,.txt" required class="w-full text-sm text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 mb-4">
                        <button type="submit" :disabled="importing" class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50">
