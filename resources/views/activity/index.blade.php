...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-heading">Activity Log</h1>
            <p class="text-muted mt-1">Track all system actions and user activities</p>
        </div>
    </div>
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="font-medium text-heading">{{ $activity->user->name ?? 'System' }}</span>
                                    @php
                                        $actionColors = [
                            ...
                            <div class="text-right flex-shrink-0">
                                <p class="text-xs text-muted whitespace-nowrap">{{ $activity->created_at->diffForHumans() }}</p>
                                @if($activity->ip_address)
                                    <p class="text-xs text-muted mt-0.5 font-mono">{{ $activity->ip_address }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
