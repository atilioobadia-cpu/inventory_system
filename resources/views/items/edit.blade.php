...
<div x-data="{ imagePreview: '{{ $item->image ? asset('storage/' . $item->image) : '' }}' }">
    <form method="POST" action="{{ route('items.update', $item) }}" enctype="multipart/form-data">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-heading">Edit Item</h1>
            <div class="flex gap-3">
                <a href="{{ route('items.show', $item) }}" class="px-4 py-2.5 text-sm font-medium text-body bg-control-bg rounded-lg hover:bg-gray-200 transition-colors">
                    Cancel
                </a>
        ...
        <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-border">
            <a href="{{ route('items.show', $item) }}" class="px-6 py-2.5 text-sm font-medium text-body bg-control-bg rounded-lg hover:bg-gray-200 transition-colors">
                Cancel
            </a>
