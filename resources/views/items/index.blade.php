...
<div x-data="{ deleteModal: false, deleteUrl: '', deleteName: '' }">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('items.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {{-- Category Filter --}}
                <select name="category_id" class="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric text-gray-600">
                ...
                {{-- Status Filter --}}
                <select name="is_active" class="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-electric/50 focus:border-electric text-gray-600">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="bg-navy text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-navy-light transition-colors">
                        Filter
                    </button>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('items.show', $item) }}" class="p-1.5 text-gray-400 hover:text-electric rounded-lg hover:bg-blue-50 transition-colors" title="View">
                                ...
                                @can('edit_items')
                                <a href="{{ route('items.edit', $item) }}" class="p-1.5 text-gray-400 hover:text-electric rounded-lg hover:bg-blue-50 transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <div class="py-16 text-center">
            ...
            @can('create_items')
            <a href="{{ route('items.create') }}" class="inline-flex items-center gap-2 bg-electric text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-600 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
                Add Item
            ...
            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6"
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <form :action="deleteUrl" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg hover:bg-red-600 transition-colors">
                            Delete
                        </button>
                    </form>
                </div>
