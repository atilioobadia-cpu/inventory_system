...
<div x-data="{ deleteModal: false, deleteUrl: '', deleteName: '' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-heading">Items</h1>
    ...
    <div class="bg-white rounded-xl border border-border p-4 mb-6">
        <form method="GET" action="{{ route('items.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {{-- Category Filter --}}
                <select name="category_id" class="border border-border rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent text-gray-600">
                ...
                {{-- Status Filter --}}
                <select name="is_active" class="border border-border rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent text-gray-600">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="bg-accent text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-accent-light transition-colors">
                        Filter
                    </button>
                    <a href="{{ route('items.index') }}" class="bg-control-bg text-gray-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        Clear
                    </a>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl border border-border overflow-hidden">
                    ...
                    <tr class="hover:bg-card-bg transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-control-bg rounded-lg flex items-center justify-center flex-shrink-0">
                                    @if($item->image)
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-danger-light text-red-700">Out of Stock</span>
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-50 text-success">In Stock</span>
                            @else
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('items.show', $item) }}" class="p-1.5 text-muted hover:text-accent rounded-lg hover:bg-primary-light transition-colors" title="View">
                                ...
                                @can('edit_items')
                                <a href="{{ route('items.edit', $item) }}" class="p-1.5 text-muted hover:text-accent rounded-lg hover:bg-primary-light transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <div class="py-16 text-center">
            ...
            @can('create_items')
            <a href="{{ route('items.create') }}" class="inline-flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-primary-dark transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
                Add Item
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-xl max-w-md w-full p-6"
                ...
                <div class="flex justify-end gap-3">
                    <button @click="deleteModal = false" class="px-4 py-2 text-sm font-medium text-body bg-control-bg rounded-lg hover:bg-gray-200 transition-colors">
                        Cancel
                    </button>
