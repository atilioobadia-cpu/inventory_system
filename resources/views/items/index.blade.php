...
<span class="mx-2">/</span>
<span class="text-gray-900">All Items</span>
@endsection
...
<div x-data="{ deleteModal: false, deleteUrl: '', deleteName: '' }">
    ...
    <div class="page-header mb-6">
        <div class="page-title">
            ...
            <p class="page-subtitle">{{ $items->total() ?? 0 }} items found</p>
        </div>
        @can('create_items')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search items..."
                           class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>

                {{-- Category Filter --}}
                <select name="category_id" class="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-gray-700">
                ...
                {{-- Status Filter --}}
                <select name="is_active" class="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <a href="{{ route('items.show', $item) }}" class="font-medium text-gray-900 hover:text-primary">{{ $item->name }}</a>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-gray-700 font-mono text-xs">{{ $item->sku }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $item->category->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-right text-gray-700">TZS {{ number_format($item->cost_price) }}</td>
                        <td class="px-4 py-3 text-right font-medium text-gray-900">TZS {{ number_format($item->selling_price) }}</td>
                        <td class="px-4 py-3 text-right font-medium {{ $item->current_stock <= 0 ? 'text-red-600' : ($item->current_stock <= ($item->reorder_point ?? 0) ? 'text-amber-600' : 'text-gray-900') }}">
                            {{ $item->current_stock }}
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('items.show', $item) }}" class="p-1.5 text-gray-500 hover:text-primary rounded-lg hover:bg-gray-100 transition-colors" title="View">
                                ...
                                @can('edit_items')
                                <a href="{{ route('items.edit', $item) }}" class="p-1.5 text-gray-500 hover:text-primary rounded-lg hover:bg-gray-100 transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                ...
                                <button @click="deleteModal = true; deleteUrl = '{{ route('items.destroy', $item) }}'; deleteName = '{{ $item->name }}'"
                                        class="p-1.5 text-gray-500 hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors" title="Delete">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
