                ...
                <div class="flex flex-col sm:flex-row gap-6">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <h2 class="text-xl font-bold text-heading">{{ $item->name }}</h2>
                                <p class="text-sm text-muted mt-0.5">SKU: <span class="font-mono">{{ $item->sku }}</span></p>
                                @if($item->barcode)
                                <p class="text-sm text-muted">Barcode: <span class="font-mono">{{ $item->barcode }}</span></p>
                                @endif
                            </div>
                            @if($item->is_active)
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-heading">{{ $item->category->name ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-muted mb-1">Supplier</p>
                                <p class="text-sm font-medium text-heading">{{ $item->supplier->name ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-muted mb-1">Cost Price</p>
                                <p class="text-sm font-medium text-heading">TZS {{ number_format($item->cost_price) }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-muted mb-1">Selling Price</p>
                                <p class="text-sm font-medium text-heading">TZS {{ number_format($item->selling_price) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            <tr>
                                ...
                                <td class="py-3 text-right text-heading font-medium">{{ $movement->balance_after }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-muted">No stock movements recorded</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-border p-6">
                ...
                <div class="mt-4 space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="text-heading">{{ $item->min_stock }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-muted">Max Stock</span>
                        <span class="text-heading">{{ $item->max_stock }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-muted">Reorder Point</span>
                        <span class="text-heading">{{ $item->reorder_point }}</span>
                    </div>
                </div>
                ...
                <div class="mt-4 p-3 bg-warning-light rounded-lg">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-sm font-medium text-warning">Stock is low. Consider reordering.</p>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl border border-border p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium text-heading">TZS {{ number_format($item->cost_price) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium text-heading">{{ $item->tax_rate }}%</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-border p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <div>
                            <p class="text-heading">{{ $purchase->reference }}</p>
                            <p class="text-xs text-muted">{{ $purchase->created_at->format('d M Y') }}</p>
                        </div>
                        <span class="text-heading">{{ $purchase->pivot->quantity }}</span>
                    </div>
                    @empty
                    <p class="text-sm text-muted text-center py-4">No purchase history</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-border p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <div>
                            <p class="text-heading">{{ $sale->reference }}</p>
                            <p class="text-xs text-muted">{{ $sale->created_at->format('d M Y') }}</p>
                        </div>
                        <span class="text-heading">{{ $sale->pivot->quantity }}</span>
                    </div>
                    @empty
                    <p class="text-sm text-muted text-center py-4">No sales history</p>
                    @endforelse
                </div>
            </div>
        </div>
