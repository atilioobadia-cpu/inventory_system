...
<span class="mx-2">/</span>
<span class="text-gray-900">{{ $item->name ?? 'Details' }}</span>
@endsection
...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="card card-body">
                <div class="flex flex-col sm:flex-row gap-6">
                    ...
                    <div class="w-full sm:w-48 h-48 bg-white rounded-lg flex items-center justify-center flex-shrink-0">
                        ...
                        @else
                        <svg class="w-16 h-16 text-gray-500" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            @else
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-white text-gray-700">Inactive</span>
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">TZS {{ number_format($item->cost_price) }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Selling Price</p>
                                <p class="text-sm font-medium text-gray-900">TZS {{ number_format($item->selling_price) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {{-- Stock Movements --}}
            <div class="card card-body">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Stock Movements</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            <tr>
                                <td class="py-3 text-gray-700">{{ $movement->created_at->format('d M Y H:i') }}</td>
                                <td class="py-3">
                                ...
                                <td class="py-3 text-gray-700 font-mono text-xs">{{ $movement->reference }}</td>
                                ...
                                <td class="py-3 text-right text-gray-900 font-medium">{{ $movement->balance_after }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-gray-500">No stock movements recorded</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-4">
            ...
            <div class="card card-body">
                ...
                <div class="mt-4 space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="text-gray-900">{{ $item->min_stock }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Max Stock</span>
                        <span class="text-gray-900">{{ $item->max_stock }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Reorder Point</span>
                        <span class="text-gray-900">{{ $item->reorder_point }}</span>
                    </div>
                </div>
            ...
            <div class="card card-body">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-900">TZS {{ number_format($item->cost_price) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Selling Price</span>
                        <span class="font-medium text-gray-900">TZS {{ number_format($item->selling_price) }}</span>
                    </div>
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-900">{{ $item->tax_rate }}%</span>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-body">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="text-gray-900">{{ $purchase->pivot->quantity }}</span>
                    </div>
                    @empty
                    <p class="text-sm text-gray-500 text-center py-4">No purchase history</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="card card-body">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <div>
                            <p class="text-gray-900">{{ $sale->reference }}</p>
                            <p class="text-xs text-gray-500">{{ $sale->created_at->format('d M Y') }}</p>
                        </div>
                        <span class="text-gray-900">{{ $sale->pivot->quantity }}</span>
                    </div>
                    @empty
                    <p class="text-sm text-gray-500 text-center py-4">No sales history</p>
                    @endforelse
                </div>
            </div>
        </div>
