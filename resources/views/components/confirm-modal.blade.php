...
<div x-data="{ {{ $name }}: false, confirmUrl: '', confirmName: '' }"
    ...
    <div x-show="{{ $name }}" x-cloak
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            ...
            {{-- Modal --}}
            <div class="relative bg-white rounded-lg max-w-md w-full p-5 z-10"
                 x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex justify-end gap-3">
                    <button @click="{{ $name }} = false"
                            class="px-4 py-2 text-sm font-medium text-body bg-white rounded-lg hover:bg-white transition-colors">
                        Cancel
                    </button>
